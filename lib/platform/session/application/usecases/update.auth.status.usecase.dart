...
class UpdateAuthStatusUseCase implements IUseCase<({AuthStatus status, String? userId}), SessionStateEntity> {
  final ISessionRepository _repository;

  const UpdateAuthStatusUseCase(this._repository);

  @override
  Future<Either<Failure, SessionStateEntity>> execute(({AuthStatus status, String? userId}) request) async {
